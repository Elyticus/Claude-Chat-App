...
// ─── Orb customize panel (Pro) ───────────────────────────────────────────────
// Live controls for the Light-mode Orb background. Floats over the hub (no
...
// CustomizePanel / GalaxyCustomizePanel but with the Orb's parameters.
...
export function OrbCustomizePanel({ settings, onChange, onReset, onClose, isDark }) {
  ...
  const hueSwatch = `hsl(${settings.hue}, 80%, 60%)`;
    ...
    >
      ...
      <div className="flex flex-col gap-3">
        ...
        <Slider
          ...
          max={2}
          step={0.05}
          onChange={(v) => set({ hoverIntensity: v })}
          isDark={isDark}
        ...
        >
          <Toggle
            ...
            checked={!!settings.forceHoverState}
            onChange={(v) => set({ forceHoverState: v })}
            isDark={isDark}
          />
          <Toggle
            label="Rotate"
            checked={!!settings.rotateOnHover}
            onChange={(v) => set({ rotateOnHover: v })}
            isDark={isDark}
            disabled={!settings.forceHoverState}
          />
        </div>
      </div>
