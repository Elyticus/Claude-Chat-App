...
│   ├── OrbitalHub.jsx           # Full-screen radial orbital canvas (room nodes) — renders AllChatsPanel. Top-right keeps profile + special-mode + light/dark toggle + bg play/stop; the rest (friends, search, plans, customize) live in one bottom-centre bubble that fans them out left/right when tapped — **mobile only** (`sm:hidden`); on desktop those same menuItems render inline in the top-right bar (menuItems + renderMenuButton; .animate-fan-left/right)
