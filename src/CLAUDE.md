...
│   ├── OrbitalHub.jsx           # Full-screen radial orbital canvas (room nodes) — renders AllChatsPanel. Top-right keeps profile + special-mode + light/dark toggle + bg play/stop; the rest (friends, search, plans, customize) live in one bottom-centre bubble. Exception: **Lite** users (canSpecial && !canCustomize) get special-mode in the bubble on mobile instead of the top bar (Pro keeps it on top everywhere; desktop always top since the bubble is mobile-only) that fans them out left/right when tapped — **mobile only** (`sm:hidden`); on desktop those same menuItems render inline in the top-right bar — except Customize, which gets its own bottom-right button next to where its panel opens (menuItems + renderMenuButton; .animate-fan-left/right)
...
- **Animation loops are capped to ~30fps to limit battery/heat on mobile** — the
  ...
  caps; if you add another hub animation loop, gate it the same way. The three
  background components (`Orb` / `Galaxy` / `Lightfall`) are wrapped in
  `React.memo` so the hub's ~30fps rotation re-render doesn't re-run their bodies
  each tick — their props (a settings object + booleans) are stable refs, so memo
  skips them; keep it that way (don't pass freshly-built object/array props).
