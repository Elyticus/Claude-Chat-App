...
│   │   ├── ChatBackdrop.jsx         # Static per-room-type faint line-art universe doodle wallpaper; rendered in ChatPanel behind the WHOLE chat box (zIndex -1 under the isolate:isolate column) so it spans header→messages→composer (bars are translucent via withAlpha). Varied icons (planets, ringed planets, crescents, comets, rockets, UFOs, satellites, 5-pt stars, orbits, galaxies, atoms, aliens, asteroids, suns, sparkles); dm/group/channel differ by icon mix + rose/amber/emerald accent. Tiled <pattern> + full-size <rect>. No CSS filter/transform/animation (caret-blink/perf safe)
